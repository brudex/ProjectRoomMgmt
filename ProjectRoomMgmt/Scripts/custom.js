(function($) {
    ...
    var isNotIndexPage = false;
    $(document).ready(function() {
        isNotIndexPage = $(".about_slider-area2").length > 0;
...
/***********my Drop down toggle custom for country in header**************/	
    $(document).ready(function (e) {
        ...
        /******************Location detector and footer fill*******************/
        var countryCode = $('#_country__Code').val();
        if (countryCode) {
            var ctryNameId = "_ctry_link_" + countryCode.toLocaleLowerCase();
            var countryName = $("#" + ctryNameId).text();
            console.log('the country name is >>', countryName);
            $("#_ctry_name_").html(countryName);
        }
        $.getJSON("/utils/CountryContact", function (data) {
            $("#_footerEmail").text(data.contactEmail);
            $("#_streetAddress").text(data.streetAddress);
            $("#_contactPhone").text(data.contactPhone);
            var countryCode = $('#_country__Code').val();
            console.log(countryCode);
            if (countryCode === null || countryCode===undefined) {
                console.log('Is undefined');
                countryCode = data.countryCode;
                console.log('The country code is now >>>', countryCode); 
            }
            if (countryCode) {
                var ctryNameId = "_ctry_link_" + countryCode.toLocaleLowerCase();
                var countryName = $("#" + ctryNameId).text();
            ...
            $("._coinLink").each(function () {
                var item = $(this);
                var link = item.attr("href");
                var href = link + countryCode;
                item.attr("href", href); 
            });
        });
